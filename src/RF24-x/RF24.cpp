...
uint8_t RF24::read_register(uint8_t reg)
{
  ...
  uint8_t result = mraa_spi_write(m_spi, 0xff);
...
uint8_t RF24::write_payload(const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  status = mraa_spi_write(m_spi,  W_TX_PAYLOAD );
  while ( data_len-- )
...
uint8_t RF24::flush_rx(void)
{
  ...
  csn(LOW);
  status = mraa_spi_write(m_spi, FLUSH_RX );
  csn(HIGH);
...
uint8_t RF24::flush_tx(void)
{
  ...
  csn(LOW);
  status = mraa_spi_write(m_spi, FLUSH_TX );
  csn(HIGH);
...
uint8_t RF24::get_status(void)
{
  ...
  csn(LOW);
  status = mraa_spi_write(m_spi, NOP );
  csn(HIGH);
