...
bool RF24Network::write(RF24NetworkHeader& header, const void* message,
	...
	while (fragment_id > 0) {
		...
		{
			delay(100);
			ok = _write(fragmentHeader, message + offset, fragmentLen, writeDirect);
			if (!ok) {
				delay(150);
				ok = _write(fragmentHeader, message + offset, fragmentLen, writeDirect);
			}
...
// Provided the to_node and directTo option, it will return the resulting node and pipe
	...
	/******************************************************************/
	uint16_t
	RF24Network::addressOfPipe(uint16_t node, uint8_t pipeNo) {
	...
	uint16_t
	RF24Network::direct_child_route_to(uint16_t node) {
	...
	uint64_t
pipe_address(uint16_t node, uint8_t pipe) {
