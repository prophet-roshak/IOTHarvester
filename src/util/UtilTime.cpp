...
int timeInit(void)
{
    ...
    if ( ret < 0 ){
        std::cout << "unable to read cpuinfo !" << std::endl;
        close(cpufreq_fd);
        return -1;
    }
    close(cpufreq_fd);
    ...
    if (!str){
        std::cout << "Buffer " << buf << " does not contain CPU frequency info !" << std::endl;
        return -1;
    }
    ...
    if(str2 == buf + sizeof(buf-1) && *str2 !='\n'){
        std::cout << "malformed cpufreq string " << str << std::endl;
        return -1;
    }
    *str2 = '\0';
    cpufreq = atof(str);
    ...
    std::cout << "cpufrequency is " << cpufreq << " mhz" << std::endl;

    /* Calculate nanoseconds per clock */
    clocks_per_ns = 1000/cpufreq;

    std::cout << "nanoseconds per clock " << clocks_per_ns << std::endl;
